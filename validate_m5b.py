...
def main():
    ...
    validation_data["hashes"]["results.json"] = hash_file("m5b_results/results.json")
    validation_data["hashes"]["evidence.jsonl"] = hash_file("m5b_results/evidence.jsonl")
    
    os.makedirs("m5b_results", exist_ok=True)
    with open("m5b_results/validation.json", "w", newline='\n') as f:
    ...
    if validation_data["pytest_exit_code"] != 0 or validation_data["viewer_build_exit_code"] != 0:
        ...
        print("Tests output:", res_tests.stdout)
