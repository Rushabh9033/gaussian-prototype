...
## Benchmark Protocol
- **Controlled Reconstruction**: Downsampled the $360\times220$ Porsche source to 2x, 4x, and 8x inputs via area sampling, then reconstructed back to $360\times220$ to evaluate PSNR, SSIM, Edge F1, Ringing, and Color Histogram Distances. Regional metrics were explicitly added for critical regions (wheel, headlamp, plate).
...
|--------|---------|-------|-------|-------|
| PSNR   | **26.84** | 24.58 | 25.21 | 25.44 |
| SSIM   | **0.860** | 0.715 | 0.754 | 0.786 |
| Edge F1| **0.878** | 0.573 | 0.656 | 0.729 |
| Ringing| 0.42% | 1.49% | 0.68% | **0.29%** |

### Actual Configuration Selection
- **Selected Configuration**: **None**. 
- **Diagnostic Configuration**: **EDR_C** (Used only for generating experimental outputs for observation, explicitly not accepted).
- **Reason**: The benchmark rules explicitly disqualified all EDR configurations due to severe losses across PSNR, SSIM, and structural metrics. No configuration was automatically chosen.
...
**Explanation**: 
This specific EDR implementation failed. It produced excessive smoothing and structural deformation and did not beat Lanczos. Missing information cannot be magically recovered by reshaping the interpolation kernel. While a structure tensor can steer a filter, any aggressive filtering along an edge simply sharpens the blockiness of a low-res image instead of inventing the high-res curvature. Note that this failure applies strictly to this implementation and does not prove that every possible mathematical method must produce identical results.
